...
from tst.wallet.secret_key_store import SecretKeyStore
from tst.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_tst(self, tst_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if tst_amount < 0:
            utxos = await self.select_coins(abs(tst_amount), exclude)
        else:
        ...
        tst_amount = spend_value + tst_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": tst_amount}]
