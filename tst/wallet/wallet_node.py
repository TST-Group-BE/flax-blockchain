...
from tst.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from tst.protocols.protocol_message_types import ProtocolMessageTypes
from tst.protocols.wallet_protocol import (
...
from tst.server.node_discovery import WalletPeers
...
from tst.util.ints import uint32, uint128
from tst.util.keychain import Keychain
from tst.util.lru_cache import LRUCache
from tst.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from tst.wallet.util.backup_utils import open_backup_file
from tst.wallet.util.wallet_types import WalletType
from tst.wallet.wallet_action import WalletAction
from tst.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'tst keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSTstConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSTstConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSTstConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSTstConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSTstConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSTstConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
