...
ROOT = pathlib.Path(importlib.import_module("tst").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `tst` file name (which is the executable)
    and the `tst` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `tst` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("tst-blockchain"))[0]
...
# TODO: collapse all these entry points into one `tst_exec` entrypoint that accepts the server as a parameter

entry_points = ["tst.cmds.tst"] + [f"tst.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "tst.util.bip39"])

if THIS_IS_WINDOWS:
    tst_mod = importlib.import_module("tst")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/tst/util/english.txt", "tst/util"))
datas.append((f"{ROOT}/tst/util/initial-config.yaml", "tst/util"))
datas.append((f"{ROOT}/tst/wallet/puzzles/*.hex", "tst/wallet/puzzles"))
datas.append((f"{ROOT}/tst/ssl/*", "tst/ssl"))
...
add_binary("tst", f"{ROOT}/tst/cmds/tst.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/tst/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/tst/server/start_{server}.py", COLLECT_ARGS)
