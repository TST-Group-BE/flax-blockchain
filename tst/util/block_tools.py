...
from tst.cmds.init_funcs import create_all_ssl, create_default_tst_config
from tst.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from tst.plotting.create_plots import create_plots
from tst.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
...
from tst.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from tst.types.generator_types import BlockGenerator, CompressorArg
...
from tst.util.config import load_config, save_config
...
from tst.util.vdf_prover import get_vdf_info_and_proof
from tst.util.wallet_tools import WalletTool
from tst.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `tst generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "xfx")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("TST_ROOT", "~/.tst/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
