...
async def disconnect_all_and_reconnect(server: TstServer, reconnect_to: TstServer) -> bool:
...
async def add_dummy_connection(server: TstServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.tst_ca_crt_path.read_bytes(), server.tst_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.tst_ca_crt_path, server.tst_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: TstServer, server_2: TstServer) -> WSTstConnection:
    """
